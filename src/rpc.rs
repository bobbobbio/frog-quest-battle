...
pub async fn start() -> Result<(), JsValue> {
    ...
    log::info!("pc1 created: state {:?}", pc1.signaling_state());
    let pc2 = RtcPeerConnection::new()?;
    log::info!("pc2 created: state {:?}", pc2.signaling_state());
    ...
    log::info!("dc1 created: label {:?}", dc1.label());
    ...
    let ondatachannel_callback = Closure::wrap(Box::new(move |ev: RtcDataChannelEvent| {
        let dc2 = ev.channel();
        log::info!("pc2.ondatachannel!: {:?}", dc2.label());

        let onmessage_callback =
            Closure::wrap(
                Box::new(move |ev: MessageEvent| match ev.data().as_string() {
                    Some(message) => log::warn!("{:?}", message),
                    None => {}
                }) as Box<dyn FnMut(MessageEvent)>,
            );
    ...
    let onicecandidate_callback1 =
        Closure::wrap(
            Box::new(move |ev: RtcPeerConnectionIceEvent| match ev.candidate() {
                Some(candidate) => {
                    log::info!("pc1.onicecandidate: {:#?}", candidate.candidate());
                    let _ =
    ...
    let onicecandidate_callback2 =
        Closure::wrap(
            Box::new(move |ev: RtcPeerConnectionIceEvent| match ev.candidate() {
                Some(candidate) => {
                    log::info!("pc2.onicecandidate: {:#?}", candidate.candidate());
                    let _ =
    ...
    log::info!("pc2: state {:?}", pc2.signaling_state());

    let answer = JsFuture::from(pc2.create_answer()).await?;
    let answer_sdp = Reflect::get(&answer, &JsValue::from_str("sdp"))?
        .as_string()
        .unwrap();
    log::info!("pc2: answer {:?}", answer_sdp);
